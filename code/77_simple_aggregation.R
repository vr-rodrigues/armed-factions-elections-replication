# 77_simple_aggregation.R -- CS-DID aggte(simple), 3 xformla specs
...
cat("============================================================\n")
cat("77_simple_aggregation.R — CS-DID aggte(simple), 3 xformla specs\n")
cat("============================================================\n\n")
...
for (ds in dummy_specs) {
  ...
  for (sp in faction_specs) {
    ...
    xf_nyt <- ds$xf
    if (ds$tag == "no_covariate") xf_nyt <- ~1
...
setcolorder(tab, c("dummy_spec", "spec", "cargo", "outcome", "estimator",
                    ...
                    "n_locs", "n_cohorts"))
fwrite(tab, file.path(OUT_DIR, "tab77_simple.csv"))
...
for (ds_tag in c("no_covariate", "capital_only", "capital_baixada")) {
  for (cg in cargos) {
    for (oc in outcomes) {
      ...
      fn <- sprintf("fig77_%s_coefplot_%s_%s.pdf", ds_tag, tolower(cg), oc)
