# ============================================================================
# 70_main_results.R — MAIN RESULTS: CS-DID NT vs NYT comparison
# ============================================================================
#
# Two estimators compared:
#   NT  (Never-Treated): control = locations OUTSIDE any faction territory
#       → more pre-treatment periods, but controls are fundamentally different
#   NYT (Not-Yet-Treated): control = same-faction locations treated later
#       → better control group, but fewer estimable cohorts
#
# Preferred: NYT (better parallel trends assumption)
#
# Unit of analysis: voting location (loc_id)
# Treatment: location inside faction territory (annual controle maps)
# Weights: qt_aptos (eligible voters)
# Base period: universal (Callaway & Sant'Anna 2021)
# allow_unbalanced_panel: TRUE
#
# Specifications:
#   A. Militia (stable_Militia locations)
#   B. Drug factions — all combined (stable_Drug locations)
#   C. Comando Vermelho only (stable_Drug + faction_specific == "CV")
#
# Outcomes × Cargo:
#   HHI, ENC, margin_victory × {Prefeito, Vereador}
#
# Output:
#   tab70_main_overall.csv     — Overall ATTs + Wald pre-trends (NT + NYT)
#   fig70_coefplot_{spec}.pdf  — Coefficient plots (NT vs NYT)
# ============================================================================
...
cat("============================================================\n")
cat("70_main_results.R — CS-DID NT vs NYT\n")
cat("============================================================\n\n")
...
dt_loc2t <- fread(file.path(BASE_DIR, "data",
...
# Map annual gvar to election year gvar_cs
map_gvar <- function(g) {
  ifelse(g == 0 | is.na(g), 2008,  # gvar=0 = inside since 2007 → first cohort
    sapply(g, function(x) {
...
# Specific faction (CV, TCP, ADA, Militia) from first inside observation
...
# Faction locations: classification + gvar + territory + specific faction
loc_info <- merge(
...
# Panel B: faction + never-treated (for NT)
# Never-treated = locations in dt_elec but NOT in any faction territory
...
cat(sprintf("Full panel (+ never-treated): %d obs, %d locations (%d never-treated)\n\n",
...
fwrite(tab, file.path(OUT_DIR, "tab70_main_overall.csv"))

# ── 7. Coefficient Plots ─────────────────────────────────────────────────
# One plot per outcome × cargo (6 plots), each with Militia + Drug as rows
# NT = filled black square, NYT = filled grey triangle
...
for (cg in cargos) {
  for (oc in outcomes) {
    ...
    fn <- sprintf("fig70_coefplot_%s_%s.pdf", tolower(cg), oc)
...
cat("\n============================================================\n")
cat("MAIN RESULTS — CS-DID NT vs NYT\n")
cat("============================================================\n\n")
