# 76_capital_baixada_dummies.R -- CS-DID with capital and baixada dummies
...
cat("============================================================\n")
cat("76_capital_baixada_dummies.R — CS-DID with capital and baixada dummies\n")
cat("============================================================\n\n")
...
run_csdid <- function(d_run, outcome, lbl, ctrl_group, xf) {
  ...
  tryCatch({
    ...
    data.table(
      att = round(agg$overall.att, 5), se = round(agg$overall.se, 5),
...
dummy_specs <- list(
  list(tag = "capital_only",    xf = ~capital),
  list(tag = "capital_baixada", xf = ~capital + baixada)
)
...
for (ds in dummy_specs) {
  ...
  for (sp in faction_specs) {
    ...
    if (ds$tag == "capital_baixada") {
      if (n_cap_nyt < 2 && n_bai_nyt < 2) {
        cat("  WARNING: no variation in either dummy, using ~1\n")
        xf_nyt <- ~1
      } else if (n_cap_nyt < 2) {
        cat("  WARNING: no capital variation in NYT, using ~baixada\n")
        xf_nyt <- ~baixada
      } else if (n_bai_nyt < 2) {
        cat("  WARNING: no baixada variation in NYT, using ~capital\n")
        xf_nyt <- ~capital
      }
    }
...
setcolorder(tab, c("dummy_spec", "spec", "cargo", "outcome", "estimator",
                    ...
                    "n_locs", "n_cohorts"))
fwrite(tab, file.path(OUT_DIR, "tab76_capital_baixada.csv"))
...
for (ds_tag in c("capital_only", "capital_baixada")) {
  for (cg in cargos) {
    for (oc in outcomes) {
      ...
      p <- ggplot(d_plot, aes(x = x_pos, y = att, shape = estimator,
        ...
        geom_errorbar(aes(ymin = ci_lo, ymax = ci_hi), width = 0.12, linewidth = 0.42) +
        geom_point(size = 2.5) +
      ...
      fn <- sprintf("fig76_%s_coefplot_%s_%s.pdf", ds_tag, tolower(cg), oc)
...
cat("\nDONE -- 76_capital_baixada_dummies.R\n")
